#!/bin/bash
#
#  willpower.sh — "my man on willpower"
#
#  Installs everything brina.sh needs before it can pull an all-nighter.
#  Run this once, then run ./brina.sh. Needs sudo on Linux.
#
#  Deps, no nonsense: aria2 · python3 · busybox · zip/unzip · p7zip
#                     · openjdk-21 · zstd · bc · xmlstarlet


if [ "$(uname -m)" == "x86_64" ] && [  "$(uname)" == "Linux" ] && [ -f "/bin/apt" ];then
    if [ "$(id -u)" != "0" ] && [ "$(uname -m)" == "x86_64" ]  && [  "$(uname)" == "Linux" ];then
        echo "🎀 Restarting as root -- I need the willpower AND the permissions"
        exec sudo /bin/bash "$0" "$@"
        exit $?
    fi
    echo "💅 Device arch: Linux x86_64 (Debian based) -- espresso incoming"
    apt update -y
    apt upgrade -y
...
if [ "$(uname -m)" == "x86_64" ] && [  "$(uname)" == "Linux" ] && [ -f "/bin/pacman" ];then
    echo "💅 Device arch: Linux x86_64 (Arch based) -- btw"
    if [ ! -f "/bin/yay" ];then
        echo "✨ Installing yay. Manual intervention may be required -- please please please cooperate."
...
if [ "$(uname -m)" == "aarch64" ];then
    echo "💅 Device arch: aarch64 -- short n' sweet"
...
if [ "$(uname)" == "Darwin" ] && [ "$(uname -m)" == "x86_64" ];then
    echo "💅 Device arch: macOS x86_64 -- taste"
