...
# Tested   : OnePlus 15 (OOS 16.0.3.501) · OnePlus 15 (CN) (COS 16.1)
# Version  : Feather Engine v1(KaoriosToolbox V2.0.3)
# ═══════════════════════════════════════════════════════════════════════════════
###############################################################################
...
fe_banner() {
    ...
    cat << 'FEATHER_ART'
╔═════════════════════════════════════════════════════════════════════════════╗
║ [×] [-] [□]  ::  T E R M I N A L  >_  S A B R I N A . E X E                 ║
╠═════════════════════════════════════════════════════════════════════════════╣
║                                                                             ║
║  SYS.MSG > "That's that me, ROM espresso."                                  ║
║                                                                             ║
║       ███████╗ █████╗ ██████╗ ██████╗ ██╗███╗   ██╗ █████╗                  ║
║       ██╔════╝██╔══██╗██╔══██╗██╔══██╗██║████╗  ██║██╔══██╗                 ║
║       ███████╗███████║██████╔╝██████╔╝██║██╔██╗ ██║███████║                 ║
...
║                                                                             ║
╟─────────────────────┬──────────────────────────────┬────────────────────────╢
║  [MODULE: TASTE]    │  [HEX_DUMP: SWEET]           │  [SYS_VARS]            ║
║  > UNLOCKING_BL...  │  0xFE 0xED 0xBE 0xEF         │  ARCH  = ARM64         ║
║  > FLASHING_IMG...  │  0x53 0x41 0x42 0x52         │  THEME = FEATHER       ║
║  > PATCHING_ROM...  │  0x49 0x4E 0x41 0x20         │  MOOD  = NONSENSE      ║
╟─────────────────────┴──────────────────────────────┴────────────────────────╢
║                                                                             ║
║  [ ♡ ] P L E A S E   P L E A S E   P L E A S E   P O R T I N G . . .        ║
║  [█████████████████████████████████░░░░░░░░░░░░░░░░] 68% // BUILDING...     ║
║                                                                             ║
╚═════════════════════════════════════════════════════════════════════════════╝
FEATHER_ART
...
if [[ -n "$portparts" ]]; then
    ...
    mix_port_part=("my_product" "my_engineering" "my_stock" "my_heytap" "my_carrier" "my_region" "my_bigball" "my_manifest")
fi
...
while IFS= read -r prop_file; do
    vndk_version=$(awk -F'=' '/^ro.vndk.version=/{print $2; exit}' "$prop_file" 2>/dev/null)
...
blue "Installing Kaorios Toolbox..."
    ...
    if [[ ! -f "${FRAMEWORK_SRC}" ]]; then
        ...
        if [[ "${KAORIOS_SKIP}" != true ]] && [[ ! -s "${KAORIOS_DEX}" ]]; then
            kaorios_dex_urls=(
                "${KAORIOS_BASE_URL}/classes_${KAORIOS_VER}.9-2.dex"
                "${KAORIOS_BASE_URL}/classes_${KAORIOS_VER}.9-1.dex"
                "${KAORIOS_BASE_URL}/classes_${KAORIOS_VER}.dex"
            )
            for kaorios_dex_url in "${kaorios_dex_urls[@]}"; do
                if wget -q --show-progress -O "${KAORIOS_DEX}" "${kaorios_dex_url}"; then
                    break
                fi
                rm -f "${KAORIOS_DEX}"
            done
            if [[ ! -s "${KAORIOS_DEX}" ]]; then
                error "Kaorios Toolbox: failed to download framework patch dex — skipping"
                KAORIOS_SKIP=true
            fi
            unset kaorios_dex_urls kaorios_dex_url
        fi
...
if [[ ${regionmark} != "CN" ]] && [[ ${base_product_model} != "IN20"* ]];then
    ...
    if [[ -f "$targetSettings" ]];then
        ...
        if [[ -n "$targetSmali" ]]; then
            python3 bin/patchmethod_v2.py "$targetSmali" isPreferenceSupport -return true
        else
            yellow "DeviceChargeInfoController.smali not found — skipping charging info patch (OOS/Global?)"
        fi
...
if [[ -n "$targetSmali" ]]; then
    ...
    yellow "SystemPropertiesHelper.smali not found — skipping RAM display patch"
fi
...
if [[ -f build/${app_patch_folder}/patched/SystemUI.apk ]]; then
    ...
    if [[ -n "$targetSmoothTransitionControllerSmali" ]]; then
        ...
        yellow "SmoothTransitionController.smali not found — skipping AOD panoramic patch"
    fi
    targetAODDisplayUtilSmali=$(find tmp/SystemUI -type f -name "AODDisplayUtil.smali")
    if [[ -n "$targetAODDisplayUtilSmali" ]]; then
        python3 bin/patchmethod_v2.py "$targetAODDisplayUtilSmali" isPanoramicProcessTypeNotSupportAllDay -return false
    else
        yellow "AODDisplayUtil.smali not found — skipping panoramic process type patch"
    fi
    if [[ $base_product_first_api_level -gt 34 ]];then
        ...
        if [[ -n "$targetStatusBarFeatureOptionSmali" ]]; then
            python3 bin/patchmethod_v2.py "$targetStatusBarFeatureOptionSmali" isChargeVoocSpecialColorShow -return true
        else
            yellow "StatusBarFeatureOption.smali not found — skipping VOOC color patch"
        fi
    fi
    if [[ $regionmark != "CN" ]];then
        ...
        if [[ -n "$targetSmali" ]]; then
            ...
            yellow "FeatureOption.smali not found — skipping My Device patch"
        fi
    fi
...
targetCommonUtilsSmali=$(find tmp -type f -path "*/com/oplus/aod/util/CommonUtils.smali")
    ...
    if [[ -n "$targetCommonUtilsSmali" ]]; then
        python3 bin/patchmethod_v2.py "$targetCommonUtilsSmali" isSupportFullAod -return true
    else
        yellow "CommonUtils.smali not found — skipping AOD full support patch"
    fi
    if [[ -n "$targetSettingsSmali" ]]; then
        python3 bin/patchmethod_v2.py "$targetSettingsSmali" getKeyAodAllDaySupportSettings -return true
    else
        yellow "SettingsUtils.smali not found — skipping AOD all-day settings patch"
    fi
...
if [[ ${port_android_version} == 16 ]] && [[ ${base_android_version} -lt 15 ]];then
    ...
    if [[ $regionmark == "CN" ]];then
        if [[ -f devices/common/wifi_fix_a16.zip ]];then
            unzip -o devices/common/wifi_fix_a16.zip -d ${work_dir}/build/portrom/images/
        else
            yellow "wifi_fix_a16.zip not found — skipping wifi fix (place in devices/common/)"
        fi
...
if [[ "$pack_method" == "stock" ]];then
    ...
    for dir in "${!prop_paths[@]}"; do
        ...
        ./bin/ota_from_target_files \
            -k "${work_dir}/otatools/key/testkey" \
