...
#  brina.sh — the ColorOS Porting Project 💋
#
#  "Say you're mine, I think it's called manifesting."
...
#  Based on Android 14.
#
#  Test Base ROM: OnePlus 9 (OxygenOS 14.0.0.1920)
#  Test Port ROM: OnePlus 15 (OxygenOS 16.0.10.500), OPPO Find X9 Ultra (ColorOS 16.0.10.500),
#                 realme GT8 Pro  (realmeUI 16.0.7.205)
#
#  Please please please read the docs before you flash. 🥺
...
# Import the caffeine — logging, patching helpers, the whole espresso shot
source caffeine.sh
...
# Check if local package or link
if [ ! -f "${baserom}" ] && [ "$(echo $baserom |grep http)" != "" ];then
    blue "Download link detected -- grabbing it, no nonsense.."
    aria2c -c --max-download-limit=1024M --file-allocation=none -s10 -x10 -j10 ${baserom}
    baserom=$(basename ${baserom} | sed 's/\?t.*//')
    if [ ! -f "${baserom}" ];then
        error "Download failed -- that link left me on read."
    fi
elif [ -f "${baserom}" ];then
...
if [ ! -f "${portrom}" ] && [ "$(echo ${portrom} |grep http)" != "" ];then
    ...
    if [ "$(/usr/bin/echo $portrom | grep downloadCheck)" != "" ];then
        blue "downloadCheck link detected! Following it to the real one..."
    ...
    green "PORTROM: ${portrom}"
else
    error "PORTROM: Invalid parameter"
    exit
fi
...
sparkle "🫀  vetting the ROMs"

blue "Validating BASEROM.. (we don't just let anyone in)"

# Detect base rom type
if unzip -l "${baserom}" | grep -q "payload.bin"; then
    ...
    error "payload.bin / *.br / *.img not found, please use official OTA or fastboot package."
    exit 1
fi

green "Detected base package type: ${baserom_type}"
...
blue "Validating PORTROM.. (bed chem pending)"
echo $portrom2
# Detect port rom type
if unzip -l "${portrom}" | grep -q "payload.bin"; then
    portrom_type="payload"
elif unzip -l "${portrom}" | grep -Eq "\.img$"; then
    portrom_type="img"
else
    error "payload.bin or *.img not found, please use an official ROM package containing system.img as PORTROM."
    exit 1
fi

# Extract version info (only if metadata exists)
...
green "ROM validation passed. Type: ${portrom_type}"
...
if [[ -n $portparts ]];then
    ...
    mix_port_part=("my_product" "my_engineering" "my_stock" "my_heytap" "my_carrier" "my_region" "my_bigball" "my_manifest" "product")
fi
...
green "ROM validation passed. They're a match."

sparkle "🧹  clearing the aftermath"
...
green "Files cleaned up. Spotless -- not a trace left."
...
if [[ ${baserom_type} == 'payload' ]]; then
    ...
    blue "Extracting files from BASEROM [*.new.dat.br]"
    unzip -q "${baserom}" -d build/baserom || \
        error "Extracting [new.dat.br] error"
    green "[new.dat.br] extracted."

    blue "Unpacking BASEROM [new.dat.br]"
    # Fix filenames with numbers
    ...
    green "[new.dat.br] unpack complete."

elif [[ ${baserom_type} == 'img' ]]; then
    blue "Extracting BASEROM containing .img files"
    mkdir -p build/baserom/images/
    unzip -q "${baserom}" -d build/baserom/tmp/ || \
        error "Extracting BASEROM error"
    # Move all img files
    find build/baserom/tmp/ -type f -name "*.img" -exec mv -fv {} build/baserom/images/ \;
    rm -rf build/baserom/tmp/
    green "[*.img] extracted."
else
    error "Unknown base package type: ${baserom_type}"
    exit 1
fi


sparkle "🎁  unpacking the PORTROM"
# ===== Extract PORTROM =====
if [[ -n ${version_name} ]] && [[ -d build/${version_name} ]]; then 
    blue "Cached ${version_name} folder detected, copying..."
    ...
    if [[ ${portrom_type} == 'payload' ]]; then
        blue "Extracting PORTROM [payload.bin]"
        ...
        blue "Extracting PORTROM containing .img files"
        # Convert comma-separated partition names to array
        IFS=',' read -ra PARTS <<< "$port_partition"
        ...
        blue "Extracting specific img files from PORTROM"

        # Only extract img files for specified partitions
        unzip -q "${portrom}" "${unzip_targets[@]}" -d "build/${version_name}/" || \
        error "Failed to extract specified img files from PORTROM."

         green "Selected partitions extracted successfully."
        find "build/${version_name}/" -type f -name "*.img" -exec cp -fv {} build/portrom/images/ \;
        green "[*.img] extracted."

    else
        error "Unknown port package type: ${portrom_type}"
        exit 1
    fi
fi

if [[ -n ${version_name2} ]] && [[ -d build/${version_name2} ]];then 
    blue "cached ${version_name2} folder detected, copying"
    #IFS=',' read -ra PARTS <<< "$port_partition"
    ...
    if [[ ${portrom2_type} == 'payload' ]]; then
        blue "Extracting files from PORTROM [payload.bin]"
...
# Unpack images
sparkle "🪶  feather-light unpacking"
green "Starting extract portrom partition from img"
for part in ${super_list};do
    # Check if in skip_list1 or skip_list2
#    if [[ " ${skip_list1[@]} " =~ " ${part} " ]] || [[ " ${skip_list2[@]} " =~ " ${part} " ]]; then
 #       yellow "Skip [${part}], already reused from patched image"
  #      continue
   # fi
    # Skip already extracted parts from BASEROM
    if [[ ! -d build/portrom/images/${part} ]]; then
        blue "Extracting [${part}]"
        ...
        yellow "Skip extracting [${part}] from PORTROM"
    fi
done
wait
rm -rf config
...
green "SDK Version: BASEROM: [SDK ${base_android_sdk}], PORTROM: [SDK ${port_android_sdk}]"
...
green "Product Model: BASEROM: [${base_product_model}], PORTROM: [${port_product_model}]"
...
green "My_Product Type: BASEROM: [${base_my_product_type}], PORTROM: [${port_my_product_type}]"
...
# base_oplusrom_version is kept for reference but is no longer used for the
# OTA card display. The OTA card now shows port_oplusrom_version (e.g. 16.0.x)
# so the version matches the port ROM, not the base.
base_oplusrom_version=$(get_oplusrom_version baserom)
[[ -z "$port_oplusrom_version" ]] && base_oplusrom_version="$port_oplusrom_version"
...
# The 32-bit RUNTIME and the 32-bit ZYGOTE are two different things and must be
# controlled separately. Conflating them cost a full debugging cycle.
#
# PORT_ENABLE_32BIT ships the 32-bit runtime: /system/bin/linker and the
# /system/lib libraries. This is NOT optional on a base device whose vendor has
# 32-bit HALs. lahaina's audio HAL is one:
#   /vendor/bin/hw/android.hardware.audio.service -> ELF class 01 (32-bit),
#   interpreter /system/bin/linker
# Without the runtime it cannot even exec ("No such file or directory" on a file
# that exists), android.hardware.audio@6.0::IDevicesFactory never registers,
# audioserver crash-loops, AudioService blocks system_server's main thread
# inside AudioSystem::setParameters, and Watchdog reboots the device roughly
# every 90s. Leave this on unless the vendor is verified 64-bit-only.
PORT_ENABLE_32BIT=${PORT_ENABLE_32BIT:-true}

# PORT_ENABLE_32BIT_ZYGOTE is what actually starts a 32-bit zygote, and it stays
# OFF because the app-side closure is not complete:
#   CANNOT LINK EXECUTABLE "/system/bin/app_process32":
#     library "liboplusplugin.so" not found
# liboplusplugin.so is not present anywhere on the donor system.img. With this
# off, ro.zygote is forced to zygote64 below even though app_process32 exists on
# disk, so the vendor's 32-bit HALs get their linker while no 32-bit zygote ever
# starts. That combination is the one that boots. Cost: no 32-bit app support.
PORT_ENABLE_32BIT_ZYGOTE=${PORT_ENABLE_32BIT_ZYGOTE:-false}

# SELinux enforcement for the vendor_boot that ships in the OTA. The port's
# policy has never been audited against this base device's vendor, and the only
# build that has ever reached the home screen did it with permissive set here.
# Enforcing turns every unreviewed denial into a hard failure, so default to the
# configuration that is known to boot and flip this once denials are triaged.
PORT_SELINUX_PERMISSIVE=${PORT_SELINUX_PERMISSIVE:-true}

# The base device's own vendor dictates the zygote config (lahaina and friends
# ship ro.zygote=zygote64_32). A 64bit-only portrom has no init.zygote64_32.rc,
# so init's "import init.${ro.zygote}.rc" resolves to nothing, no zygote is ever
# defined, and the device hangs forever on the boot animation. Restore the
# 32-bit runtime so the vendor's request can actually be served.
if [[ "$PORT_ENABLE_32BIT" == "true" && ! -f build/portrom/images/system/system/bin/app_process32 && -n "$vendor_cpu_abilist32" ]]; then
    blue "64bit only portrom detected"
    if [[ -f devices/common/32bit-compatibility.zip ]];then
        unzip -o devices/common/32bit-compatibility.zip -d build/portrom/images/ > /dev/null || error "Failed to extract 32bit-compatibility.zip"
        # the zip ships the compressed ART apex; leaving an uncompressed one
        # beside it gives apexd two copies of the same module
        rm -f build/portrom/images/system/system/apex/com.google.android.art.apex
        # contextpatch.py guesses labels from siblings; these two are too
        # important to guess. A mislabelled app_process32 means zygote_secondary
        # is denied exec and the 32-bit zygote never comes up.
        # contextpatch.py only ADDS entries it does not already have, so pinning
        # the correct label here is what stops it guessing. Verified against a
        # system.img that actually booted: without these two it labels
        # init.zygote64_32.rc as vendor_configs_file and framework/arm as
        # vendor_file, and the device dies on the boot animation - init cannot
        # use the rc, so "import init.${ro.zygote}.rc" defines no zygote at all,
        # and ART cannot read the 32-bit boot image.
        system_file_contexts="build/portrom/images/config/system_file_contexts"
        if [[ -f ${system_file_contexts} ]];then
            pin_context(){
                # $1 = escaped path as it appears in file_contexts, $2 = label
                if grep -q "^$1 " ${system_file_contexts};then
                    sed -i "s@^$1 .*@$1 $2@" ${system_file_contexts}
                else
                    echo "$1 $2" >> ${system_file_contexts}
                fi
            }
            pin_context '/system/system/bin/app_process32' 'u:object_r:zygote_exec:s0'
            pin_context '/system/system/bin/linker' 'u:object_r:system_linker_exec:s0'
            pin_context '/system/system/etc/init/hw/init\.zygote64_32\.rc' 'u:object_r:system_file:s0'
            pin_context '/system/system/framework/arm' 'u:object_r:system_file:s0'
        fi
        # A 64bit-only portrom ships ro.system.product.cpu.abilist32= (empty),
        # which is true of the donor but a lie once we inject a 32-bit runtime
        # and the vendor's ro.zygote=zygote64_32 starts a 32-bit zygote. The
        # framework reads these to build Build.SUPPORTED_32_BIT_ABIS, so
        # PackageManager resolves instruction sets against an empty 32-bit ABI
        # list while a 32-bit zygote is live. Mirror what the vendor declares -
        # the same values a natively 64_32 baserom carries in system/build.prop.
        vendor_cpu_abilist=$(< build/portrom/images/vendor/build.prop grep "^ro.vendor.product.cpu.abilist=" |awk 'NR==1' |cut -d '=' -f 2 |tr -d '\r')
        system_build_prop="build/portrom/images/system/system/build.prop"
        # Only advertise a 32-bit ABI when a 32-bit zygote is actually running.
        # Declaring it with the zygote off makes PackageManager resolve apps to an
        # instruction set nothing can execute.
        if [[ "$PORT_ENABLE_32BIT_ZYGOTE" == "true" && -n "$vendor_cpu_abilist32" && -f ${system_build_prop} ]];then
            blue "Declaring 32-bit ABI support in system/build.prop"
            sed -i "s@^ro.system.product.cpu.abilist=.*@ro.system.product.cpu.abilist=${vendor_cpu_abilist}@" ${system_build_prop}
            sed -i "s@^ro.system.product.cpu.abilist32=.*@ro.system.product.cpu.abilist32=${vendor_cpu_abilist32}@" ${system_build_prop}
        fi
        green "Injected 32-bit runtime into portrom"
    else
        yellow "devices/common/32bit-compatibility.zip not found, skipping 32-bit injection"
    fi
fi

# mediaserverwrapper execs /system/bin/mediaserver64 when this is set. Without
# it mediaserver.zygote64_32.rc points "media32" at /system/bin/mediaserver32,
# which a 64bit-only portrom does not ship, and system_server dies with it.
if [[ ! -f build/portrom/images/system/system/bin/mediaserver32 && -f build/portrom/images/system/system/bin/mediaserver64 ]];then
    blue "Forcing 64-bit mediaserver"
    sed -i "/^ro.mediaserver.64b.enable=/d" build/portrom/images/vendor/default.prop
    echo "ro.mediaserver.64b.enable=true" >> build/portrom/images/vendor/default.prop
    # No rc edit is needed. mediaserverwrapper is the "media" service and it
    # execs /system/bin/mediaserver64 directly off this prop - the media32 /
    # media64 service definitions in mediaserver.${ro.zygote}.rc are "disabled"
    # and nothing anywhere starts them by name (verified: zero references across
    # every rc in the tree). Renaming them was a no-op and was removed.
fi

# com.android.se registers android.se.omapi.ISecureElementService with VINTF
# stability, and servicemanager refuses addService for an interface that no
# VINTF manifest declares. The portrom ships SecureElement.apk and a device
# compatibility matrix that requires the interface, but not the manifest
# fragment AOSP installs alongside the apk, so the app dies with
#   java.lang.IllegalArgumentException: VINTF declaration error
# on every start and is respawned forever - 2268 crashes in the first 68
# minutes of the first build that booted. Ship the fragment.
if [[ -f build/portrom/images/system_ext/app/SecureElement/SecureElement.apk ]];then
    se_declared=$(grep -rls "android\.se\.omapi" \
        build/portrom/images/system/system/etc/vintf \
        build/portrom/images/system_ext/etc/vintf \
        build/portrom/images/product/etc/vintf \
        build/portrom/images/vendor/etc/vintf \
        build/portrom/images/odm/etc/vintf 2>/dev/null | grep -v compatibility_matrix)
    if [[ -z "$se_declared" ]];then
        blue "Declaring android.se.omapi in the system_ext VINTF manifest"
        mkdir -p build/portrom/images/system_ext/etc/vintf/manifest
        cat > build/portrom/images/system_ext/etc/vintf/manifest/android.se.omapi.xml <<"EOF"
...
EOF
        # fspatch/contextpatch only add entries they are missing, and contextpatch
        # guesses. Pin both so a file servicemanager must read cannot end up with
        # a guessed label.
        se_fs_config="build/portrom/images/config/system_ext_fs_config"
        se_file_contexts="build/portrom/images/config/system_ext_file_contexts"
        if [[ -f ${se_fs_config} ]] && ! grep -qF "system_ext/etc/vintf/manifest/android.se.omapi.xml" ${se_fs_config};then
            echo "system_ext/etc/vintf/manifest/android.se.omapi.xml 0 0 0644" >> ${se_fs_config}
        fi
        if [[ -f ${se_file_contexts} ]] && ! grep -qF "android\.se\.omapi" ${se_file_contexts};then
            echo "/system_ext/etc/vintf/manifest/android\.se\.omapi\.xml u:object_r:system_file:s0" >> ${se_file_contexts}
        fi
    fi
fi

# Whatever ro.zygote the vendor asks for, the matching init rc has to exist or
# nothing starts. Degrade to a config the portrom can actually serve instead of
# shipping an image that dies on the boot animation.
zygote_mode=$(cat build/portrom/images/vendor/*.prop 2>/dev/null | grep -m1 "^ro.zygote=" | cut -d '=' -f 2 | tr -d '\r')
zygote_fallback=""
if [[ -n "$zygote_mode" && ! -f build/portrom/images/system/system/etc/init/hw/init.${zygote_mode}.rc ]];then
    zygote_fallback="portrom has no init.${zygote_mode}.rc"
elif [[ "$zygote_mode" == *_32 && ! -f build/portrom/images/system/system/bin/app_process32 ]];then
    # an init.zygote64_32.rc pointing at a missing app_process32 is the same
    # trap as media32 pointing at a missing mediaserver32
    zygote_fallback="portrom has no app_process32 to serve ${zygote_mode}"
fi
if [[ -n "$zygote_fallback" ]];then
    yellow "${zygote_fallback}, falling back to ro.zygote=zygote64"
    for prop_file in $(grep -rl "^ro.zygote=" build/portrom/images/vendor/ 2>/dev/null);do
        sed -i "s/^ro.zygote=.*/ro.zygote=zygote64/" ${prop_file}
    done
fi

# The fallback above only fires when the rc or app_process32 is MISSING. Once
# the 32-bit runtime is injected both exist, so the vendor's ro.zygote=zygote64_32
# would survive and start a 32-bit zygote that cannot link. Force it down here:
# the runtime stays on disk for the vendor's 32-bit HALs, the zygote does not run.
if [[ "$PORT_ENABLE_32BIT_ZYGOTE" != "true" ]];then
    for prop_file in $(grep -rl "^ro.zygote=zygote64_32" build/portrom/images/vendor/ 2>/dev/null);do
        yellow "32-bit zygote disabled, forcing ro.zygote=zygote64 in $(basename ${prop_file})"
        sed -i "s/^ro.zygote=.*/ro.zygote=zygote64/" ${prop_file}
    done
fi
...
# Other devices might not have default.prop
...
if [[ -f build/${app_patch_folder}/patched/services.jar ]];then
    blue "Copying already processed services.jar"
    ...
    while read -r smali_file; do
        ...
        { sed -i "${original_line_number},${move_result_end_line}d" "$smali_file" && sed -i "${original_line_number}i\\${replace_with_command}" "$smali_file"; } && blue "${smali_file} patched"
    ...
    find tmp/services/ -type f -name "ReconcilePackageUtils.smali" | while read smali_file; do
        ...
        if [[ -n "$match_line" ]]; then
            ...
            # Insert const/4 vX, 0x1 before the line
...
if [[ -f build/${app_patch_folder}/patched/framework.jar ]];then
    blue "Copying already processed framework.jar"
    ...
    if [[ -f devices/common/0001-core-framework-Introduce-OplusPropsHookUtils-V6.patch ]]; then
        ...
        echo "🔧 Applying patch file 0001-core-framework-Introduce-OplusPropsHookUtils-V6.patch ..."
        git apply ${work_dir}/devices/common/0001-core-framework-Introduce-OplusPropsHookUtils-V6.patch && echo "✅ Patch applied successfully" || echo "❌ Patch application failed"
...
if [[ -f build/${app_patch_folder}/patched/oplus-services.jar ]];then
    blue "Copying already processed oplus-services.jar"
...
if [[ ${base_device_family} == "OPSM8250" ]] || [[ ${base_device_family} == "OPSM8350" ]]; then
    blue "COS15/OOS15: Fix Face Unlock for SM8250/8350"
    #pushd tmp/services
...
if [[ -f build/${app_patch_folder}/patched/HeyTapSpeechAssist.apk ]]; then
    blue "Copying already processed HeyTapSpeechAssist.apk"
...
if [[ $ota_patched == "false" ]];then
    ...
    if [[ -f build/${app_patch_folder}/patched/AIUnit.apk ]]; then
            blue "Copying already processed AIUnit.apk"
...
if [[ -f devices/common/xeutoolbox.zip ]] && [[ $base_android_version -lt 15 ]] && [[ ${portIsColorOSGlobal} != true ]];then
    ...
    if [[ -f build/${app_patch_folder}/patched/OppoGallery2.apk ]]; then
            blue "Copying already processed OppoGallery2"
...
if [[ ${base_device_family} == "OPSM8250" ]] || [[ ${base_device_family} == "OPSM8350" ]];then
    ...
    if [[ -f build/${app_patch_folder}/patched/Battery.apk ]]; then
        blue "Copying already processed Battery"
...
if [[ $port_android_version -ge 16 ]];then
    if [[ -f $targetSettings ]];then
        ...
        const-string v0, \"${port_oplusrom_version}\"
...
# -- Eye Comfort APK replacement --------------------------------------------------
# Replace com.oplus.eyeprotect with patched version
targetEyeProtect=$(find build/portrom/images/ -name "EyeProtect.apk" -o -name "OplusEyeProtect.apk" | head -n1)
if [[ -z "$targetEyeProtect" ]]; then
    targetEyeProtect=$(find build/portrom/images/ -path "*/com.oplus.eyeprotect/*" -name "*.apk" | head -n1)
fi
if [[ -n "$targetEyeProtect" ]] && [[ -f devices/common/eyecomfortpatch.apk ]]; then
    blue "Replacing Eye Comfort APK with patched version"
    cp -rfv devices/common/eyecomfortpatch.apk "$targetEyeProtect"
    green "Eye Comfort APK replaced"
elif [[ -z "$targetEyeProtect" ]]; then
    yellow "Eye Comfort APK not found in portrom -- skipping replacement"
else
    yellow "devices/common/eyecomfortpatch.apk not found -- skipping Eye Comfort replacement"
fi

# -- Wallpapers & Styles APK replacement ------------------------------------------
targetWallpapers=$(find build/portrom/images/ -name "Wallpaper.apk" -o -name "OplusWallpaper.apk" -o -name "OplusWallpapers.apk" | head -n1)
if [[ -z "$targetWallpapers" ]]; then
    targetWallpapers=$(find build/portrom/images/ -path "*/com.oplus.wallpapers/*" -name "*.apk" | head -n1)
fi
if [[ -n "$targetWallpapers" ]] && [[ -f devices/common/wallpapersfix.apk ]]; then
    blue "Replacing Wallpapers & Styles APK with patched version"
    cp -rfv devices/common/wallpapersfix.apk "$targetWallpapers"
    green "Wallpapers & Styles APK replaced"
elif [[ -z "$targetWallpapers" ]]; then
    yellow "Wallpapers & Styles APK not found in portrom -- skipping replacement"
else
    yellow "devices/common/wallpapersfix.apk not found -- skipping Wallpapers & Styles replacement"
fi

# -- SystemUI APK replacement -----------------------------------------------------
targetSystemUI=$(find build/portrom/images/ -name "SystemUI.apk" | head -n1)
if [[ -z "$targetSystemUI" ]]; then
    targetSystemUI=$(find build/portrom/images/ -path "*/com.android.systemui/*" -name "*.apk" | head -n1)
fi
if [[ -n "$targetSystemUI" ]] && [[ -f devices/common/SystemUI.apk ]]; then
    blue "Replacing SystemUI APK with patched version"
    cp -rfv devices/common/SystemUI.apk "$targetSystemUI"
    green "SystemUI APK replaced"
elif [[ -z "$targetSystemUI" ]]; then
    yellow "SystemUI APK not found in portrom -- skipping replacement"
else
    yellow "devices/common/SystemUI.apk not found -- skipping SystemUI replacement"
fi

# -- Settings APK replacement (ColorOS Global only) -------------------------------
if [[ "$portIsColorOSGlobal" == true ]]; then
    if [[ -f devices/common/settingspatch.apk ]]; then
        if [[ -n "$targetSettings" ]] && [[ -f "$targetSettings" ]]; then
            blue "ColorOS Global detected -- replacing Settings APK with patched version"
            cp -rfv devices/common/settingspatch.apk "$targetSettings"
            green "Settings APK replaced for ColorOS Global"
        else
            yellow "Settings.apk not found in portrom -- skipping Settings replacement"
        fi
    else
        yellow "devices/common/settingspatch.apk not found -- skipping Settings replacement"
    fi

    # -- Software Update APK replacement (ColorOS Global only) ----------------------
    targetSoftwareUpdate=$(find build/portrom/images/ -name "SoftwareUpdate.apk" -o -name "OplusOTA.apk" -o -name "OTA.apk" | head -n1)
    if [[ -z "$targetSoftwareUpdate" ]]; then
        targetSoftwareUpdate=$(find build/portrom/images/ -path "*/com.oplus.ota/*" -name "*.apk" | head -n1)
    fi
    if [[ -f devices/common/softwareupdate.apk ]]; then
        if [[ -n "$targetSoftwareUpdate" ]] && [[ -f "$targetSoftwareUpdate" ]]; then
            blue "ColorOS Global detected -- replacing Software Update APK with patched version"
            cp -rfv devices/common/softwareupdate.apk "$targetSoftwareUpdate"
            green "Software Update APK replaced for ColorOS Global"
        else
            yellow "Software Update APK not found in portrom -- skipping Software Update replacement"
        fi
    else
        yellow "devices/common/softwareupdate.apk not found -- skipping Software Update replacement"
    fi
fi

targetOplusLauncher=$(find build/portrom/images/ -name "OplusLauncher.apk")
if [[ -z "$targetOplusLauncher" ]]; then
    targetOplusLauncher=$(find build/portrom/images/ -path "*/com.oplus.launcher/*" -name "*.apk" | head -n1)
fi

# -- System Launcher APK replacement ----------------------------------------------
if [[ -n "$targetOplusLauncher" ]] && [[ -f devices/common/systemlauncherpatch.apk ]]; then
    blue "Replacing System Launcher APK with patched version"
    cp -rfv devices/common/systemlauncherpatch.apk "$targetOplusLauncher"
    green "System Launcher APK replaced"
elif [[ -z "$targetOplusLauncher" ]]; then
    yellow "System Launcher APK not found in portrom -- skipping System Launcher replacement"
else
    yellow "devices/common/systemlauncherpatch.apk not found -- skipping System Launcher replacement"
fi

if [[ -f $targetOplusLauncher ]] && [[ $base_product_first_api_level -gt 34 ]];then
     blue "Unlock RAM display"
...
if [[ -f build/${app_patch_folder}/patched/SystemUI.apk ]]; then
        blue "Copying already processed SystemUI.apk"
    ...
    blue "Unlock panoramic full screen AOD"
...
if [[ -f $targetAOD ]] && [[ $base_product_first_api_level -le 35 ]] ;then
    blue "Force enable all-day AOD for older devices"
...
sparkle "🗑️  debloat era"
yellow "Debloating... taking out the trash, and the trash is bloatware."
...
# RescueParty is what turns "system_server crashes" into "device bootloops at
# the boot animation". When system_server dies repeatedly it escalates until it
# reboots the device, so a port with a crashing framework never survives long
# enough to reach the launcher. Disabled, the framework simply restarts in place
# and the device stays usable and diagnosable.
#
# These MUST go in vendor/default.prop, not system/build.prop. The only build
# ever observed to reach the home screen carried them here; the builds that
...
if [[ $(cat build/baserom/images/my_product/build.prop | grep "ro.oplus.audio.effect.type" | cut -d "=" -f 2) == "dolby" ]] ;then
   blue "Fix Dolby + App Specific volume adjustment for SM8250/SM8350"
...
blue "Applying ringtones and notification sounds fix"
if [[ -d devices/common/sounds-fix ]]; then
    cp -rf devices/common/sounds-fix/my_product/* build/portrom/images/my_product/
    cp -rf devices/common/sounds-fix/system/system_ext/* build/portrom/images/system_ext/
fi
...
oplus_features=(
    "oplus.software.directservice.finger_flashnotes_enable^Breeno Memory" 
    "oplus.software.support_quick_launchapp"  
    "oplus.software.support_blockable_animation" 
    "oplus.software.support.zoom.multi_mode" 
    #"oplus.software.radio.networkless_support^Networkless Chat" 
    #"oplus.software.display.ai_eyeprotect_v1_support^AI Eye Protect"
    ...
    "oplus.software.multi_app.volume.adjust.support^Multi Volume Adjust" 
    "oplus.software.systemui.navbar_pick_color^15.0.2.201 Added"
    "oplus.software.string_gc_support"
    "oplus.software.display.rgb_ball_support^Color Temp Adjust Ball"
    ...
    "oplus.software.display.game_dark_eyeprotect_support^Game Night Eye Protect"
    "oplus.software.systemui.navbar_pick_color^Nav bar color pick opt"
    "oplus.software.smart_sidebar_video_assistant^Sidebar Video Assistant"
    "oplus.video.audio.volume.enhancement^Video Volume Enhance" 
    "oplus.software.display.lux_small_debounce_expand_support"
    "oplus.hardware.display.no_bright_eyes_low_freq_strobe^Low Brightness Strobe"
   ...
   "oplus.software.display.enhance_brightness_with_uidimming^LocalHDR"
    "oplus.software.adaptive_smooth_animation^Shanhai Network Engine"
...
if [[ $vndk_version -gt 33 ]];then
 add_feature_v2 oplus_feature "oplus.software.radio.networkless_support^Networkless Chat"
fi
...
app_features=(
    ...
    "com.oplus.mediaturbo.game_live^Live Assistant"
    "oplus.aod.wakebyclick.support^Click to Wake AOD"
    ...
    "com.oplus.note.aigc.ai_rewrtie.support^AI Rewrite"
    #"feature.super_settings_smart_touch_v2.support^Smart Touch V2"
    ...
    "feature.super_settings_smart_touch.support^Smart Touch V1"
    ...
    "com.oplus.smartmediacontroller.lss_assistant_enable^Sidebar Audio Track Assistant"
    # "com.android.incallui.share_screen_and_touch_cmd_support^Screen Share" causes crash on OOS call
    ...
    "com.oplus.gesture.camera_space_gesture_support^Air Gestures" # Needs RM OplusGesture App
    "com.oplus.gesture.intelligent_perception"
    "com.oplus.dmp.aiask_enable^AI Search^args=\"int:1\""
    "os.graphic.gallery.photoeditor.aibesttake^AI Best Take^args=\"int:1\""
    "com.oplus.tips.os_recommend_page_index^New Feature Recommend^args=\"String:indexOS15_0_2_new\""
    ...
    "com.oplus.securityguard.sample.feature_enable^Security Guard^args=\"boolean:true\""
...
if [[ "${base_product_device}" == "OnePlus9Pro" ]] ;then
    add_feature_v2 app_feature "os.charge.settings.wirelesscharging.power^Set wireless charging wattage^args=\"int:50\"" "oplus.power.wirelesschgwhenwired.support" "com.oplus.battery.wireless.charging.notificate" "os.charge.settings.wirelesschargingcoil.position" "os.charge.settings.wirelesscharge.support"
elif [[ "${base_product_device}" == "OP4E3F" ]] || [[ "${base_product_device}" == "OP4E5D" ]];then
    add_feature_v2 app_feature "os.charge.settings.wirelesscharging.power^Set wireless charging wattage^args=\"int:30\"" "oplus.power.wirelesschgwhenwired.support" "com.oplus.battery.wireless.charging.notificate" "os.charge.settings.wirelesschargingcoil.position" "os.charge.settings.wirelesscharge.support"
else
...
if [[ -d devices/common/GTMode/overlay ]] && [[ $port_android_version != "16" ]];then
    ...
    add_feature_v2 app_feature "com.android.settings.device_rm^Realme device, required for GT Mode"
...
# High Brightness Mode toggle switch
...
add_feature_v2  app_feature "com.oplus.fullscene_plc_charge.support^Full Scene Bypass Charging^args=\"boolean:true\""
# Alert slider
...
remove_feature "oplus.software.display.wcg_2.0_support" # Fix soft reboot when switching color mode
...
remove_feature "oplus.software.display.adfr_v32_hp"  # Fix OOS Breeno memory crash
 ...
 if [[ $base_android_version -lt 33 ]];then
    ...
    #   find build/portrom/images/ -type f -name "*.prop" -exec  grep "ro.product.model" {} \;
        ...
        if [[ -n $base_scanner_app ]] && [[ -n $target_scanner_app ]];then
                blue "Replacing Stock OcrScanner"
...
if [[ ${base_product_device} == "OnePlus8Pro" ]] ;then 
    if [[ ${port_android_version} -gt 15 ]];then
            {
    echo "# OnePlus8Pro removed properties"
...
# Unlock WeChat Moments Live Photo
...
# Devices/device_code/overlay follows the image directory structure, can replace targets directly.
...
if [[ -d "devices/${base_product_device}/overlay" ]]; then
    ...
    yellow "devices/${base_product_device}/overlay not found" 
fi
...
source devices/common/feather/feather.sh
apply_feather_engine "${base_device_family}" "${base_product_device}" "${base_product_model}"
...
for zip in $(find devices/${base_product_device}/ -name "*.zip"); do
    if unzip -l $zip | grep -q "anykernel.sh" ;then
        blue "Custom Kernel zip $zip detected [Anykernel]"
...
for anykernel_dir in tmp/anykernel*; do
    if [ -d "$anykernel_dir" ]; then
        blue "Start integrating custom kernel into boot.img"
        ...
        if [[ "$anykernel_dir" == *"-ksu"* ]]; then
            ...
            blue "New boot_ksu.img generated"
...
if [ -z "$kmi" ]; then
    echo "⚠ No matching KMI found (ro.build.kernel.id=$kernel_id), skipping ksud patch for init_boot"
else
    echo "✔ Kernel version $kernel_major detected → using KMI: $kmi"
...
# Add erofs filesystem fstab
# if [ ${pack_type} == "EROFS" ];then
#     yellow "Validating whether adding erofs mount points is needed."
...
#                         yellow "Adding mount point $pname"
#                     else
...
green "Packing img -- short n' sweet"
...
for img in $(find build/baserom/ -type f -name "vbmeta*.img");do
    blue "Disable vbmeta verify: $img"
...
if [[ $pack_method == "stock" ]];then
    ...
    # Applied to the staged image, after every other vendor_boot step, so this
    # is guaranteed to be what actually ships. patch_bootarg.py rewrites only
    # the 2048-byte cmdline field in the header - the vendor ramdisk and the dtb
    # are left byte-identical, so the first-stage modules cannot be disturbed.
    if [[ "$PORT_SELINUX_PERMISSIVE" == "true" ]] && [[ -f "out/target/product/${base_product_device}/IMAGES/vendor_boot.img" ]];then
        blue "Setting androidboot.selinux=permissive on vendor_boot"
        python3 bin/patch_bootarg.py "out/target/product/${base_product_device}/IMAGES/vendor_boot.img" androidboot.selinux=permissive || error "Failed to patch vendor_boot cmdline"
    fi
   ...
   if [[ $is_ab_device == true ]]; then
    ...
    sparkle "☕  that's that me espresso"
    green "Porting completed. Is it that sweet? I guess so."
    green "Output: "
    green "$(pwd)/out/${os_type}_${rom_version}_${hash}_${port_product_model}_${pack_timestamp}_${pack_type}.zip"
    yellow "Back up your data before you flash. Don't bring me to tears. 🥺"
fi
